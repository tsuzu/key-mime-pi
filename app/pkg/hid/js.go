...
var jsToHIDKeycodes = map[int]int{
	3:   0x48, // Pause / Break
...
type JEKeyEvent struct {
	MetaModifier  bool   `json:"metaKey"`
	AltModifier   bool   `json:"altKey"`
	ShiftModifier bool   `json:"shiftKey"`
	CtrlModifier  bool   `json:"ctrlKey"`
	Key           string `json:"key"`
...
func Convert(jsKeyEvent JEKeyEvent) (controlChars int, hidKeyCode int, ok bool) {
	controlChars = 0
	if jsKeyEvent.CtrlModifier {
		controlChars |= 1 << 0
	}
	if jsKeyEvent.ShiftModifier {
		controlChars |= 1 << 1
	}
	if jsKeyEvent.AltModifier {
		controlChars |= 1 << 2
	}
	if jsKeyEvent.MetaModifier {
		controlChars |= 1 << 3
	}
	...
	return controlChars, hidKeyCode, ok
}
